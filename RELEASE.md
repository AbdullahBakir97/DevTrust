...
This monorepo ships **five independent packages** to PyPI:

| Distribution | Source path | What it does |
|---|---|---|
| `repox`    | `src/products/01-repo-xray/code/` | Codebase architecture model (CLI + library) |
| `sts`      | `src/products/02-smart-test-selector/code/` | Test selection engine (CLI + library) |
| `sts-app`  | `src/products/02-smart-test-selector/app/` | GitHub App for `sts` |
| `apr`      | `src/products/03-agent-pr-reviewer/code/` | PR review engine (CLI + library) |
| `apr-app`  | `src/products/03-agent-pr-reviewer/app/` | GitHub App for `apr` |
| `whychanged` | `src/products/07-whychanged/code/` | Production diff-detective (Wave 3) |
| `tokencost`  | `src/products/08-tokencost/code/` | LLM cost attribution (Wave 3) |
| `agtrace`    | `src/products/06-agent-trace-sdk/code/` | Agent-aware tracing (Wave 3) |

Each package versions independently. The release workflow fires on git tags
matching `<package>-v<version>` and publishes that one package only.

## One-time setup (per package)

1. **Create the public GitHub repo.** Push the code so the package URLs in
   `pyproject.toml` resolve.
2. **Register a Trusted Publisher on PyPI.** For each project, go to
   `https://pypi.org/manage/account/publishing/` and add:
   - **Owner:** `AbdullahBakir97`
   - **Repository:** `DevTrust` (the monorepo)
   - **Workflow:** `release.yml`
   - **Environment:** `pypi-<package>` (e.g. `pypi-repox`)
3. **Create the matching GitHub Environment.** Repo settings -> Environments
   -> `pypi-<package>`. Add review gates if you want a human in the loop.
...
uv run python scripts/release.py --check --package repox

# 4. Find the right tag string
uv run python scripts/release.py --tag-suggestion repox

# 5. Commit, tag, push
git add .
git commit -m "Release repox v0.4.1"
git tag repox-v0.4.1
